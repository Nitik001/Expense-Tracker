    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-content animate-slide-up upcoming-sheet">
        ...
        <div className="flex justify-between items-center mb-5">
          <div>
            <h2 className="text-xl font-bold">Upcoming Money</h2>
            <p className="text-xs text-muted mt-1">Track money you're expecting to receive</p>
          </div>
        ...
        {!showForm ? (
          ...
          <form onSubmit={handleAdd} className="upcoming-form">
            <input className="form-input" placeholder="What is it? (e.g. Salary, Loan)" value={title} onChange={e => setTitle(e.target.value)} required />
            <input className="form-input" type="number" placeholder="Amount" value={amount} onChange={e => setAmount(e.target.value)} required />
            <input className="form-input" placeholder="From whom? (e.g. Rahul, Company)" value={from} onChange={e => setFrom(e.target.value)} />
            <input className="form-input" type="date" value={expectedDate} onChange={e => setExpectedDate(e.target.value)} />
            <div className="flex gap-3">
              <button type="button" className="btn-secondary flex-1" style={{ background: 'var(--color-surface)', color: 'var(--color-text)' }} onClick={() => setShowForm(false)}>Cancel</button>
        ...
        {pending.length > 0 && (
          <div className="upcoming-section">
            <p className="section-mini-label">Pending · {pending.length}</p>
                ...
                <div key={item.id} className="upcoming-card pending">
                  ...
                  <div className="upcoming-card-info">
                    ...
                    <span className="text-xs text-muted">
                      {item.from ? `From: ${item.from}` : 'Source not set'}
                      {item.expectedDate ? ` · Due ${item.expectedDate}` : ''}
                    </span>
                  </div>
        ...
        {received.length > 0 && (
          <div className="upcoming-section">
            <p className="section-mini-label">Received ✓</p>
              ...
              {received.map(item => (
                <div key={item.id} className="upcoming-card received">
                  ...
                  <div className="upcoming-card-info">
                    ...
                    <span className="text-xs text-muted">{item.from ? `From: ${item.from}` : ''}</span>
                  </div>
                  <div className="upcoming-card-right">
        ...
        {pending.length === 0 && received.length === 0 && !showForm && (
          <div className="upcoming-empty">
            ...
            <p className="text-sm text-muted mt-2">No upcoming income tracked yet</p>
          </div>
        )}
      </div>
    </div>
